...
def main(data_dir: str, repeats: int, do_plot: bool):
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as out:
        writer = csv.DictWriter(
            out, fieldnames=["dataset", "n", "algo", "time_s", "comparisons"]
        )
        writer.writeheader()
    ...
    if do_plot:
        try:
            ...
            for algo in ALGOS.keys():
                ...
                plt.savefig(
                    out_dir / f"bench_plot_{algo}.png", dpi=150, bbox_inches="tight"
                )
                plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Nombre de répétitions (meilleur temps conservé)",
    )
    parser.add_argument(
        "--plot", action="store_true", help="Génère des graphes (matplotlib)"
    )
